...
public final class LoggingIndexerListener implements IndexerListener {
    ...
    public void closed(final BulkIndexerStats stats) {
        LOGGER.debug("Bulk indexer {}: {}",
                stats.getSuccessful() == stats.getTotal() ? "successful" : "FAILED", stats);
    }
    ...
    @Override
    public void onFailure(final BulkOperation op, final Response<BulkResponse> result) {
        LOGGER.debug("Bulk operation failed: {}", op);
    }
