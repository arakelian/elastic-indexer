...
public interface IndexerListener {
    ...
    /**
     * Invoked when a {@code BulkOperation} fails individually. These operations always have a status
     * code outside the 2xx range, and they are generally not retryable because they have status codes
     * below the 5xx range, which are reserved for server-side issues.
     * 
     * @param op
     *            bulk operation
     * @param response
     ...
     * @param result
     *            the response from the Elastic Bulk API, always with status code outside the 2xx range.
     */
    void onFailure(BulkOperation op, Response<BulkResponse> result);

    /**
     * Invoked when a {@code BulkOperation} fails because the entire request failed, usually due to an
     * IOException. It's possible that the Elastic Bulk API never received the request.
     *
     * Note: The bulk API request would been retried according to configured
     * {@link BulkIndexerConfig#getRetryer()} policy.
     *
    ...
    /**
     * Invoked when a {@code BulkOperation} is successful. These operations usually have a status code
     * in the 2xx range, but there are exceptions, such as 404 status code for delete operations which
     * are benign and merely reported that the requested record was not found.
     * 
     * @param op
     *            bulk operation
     */
