...
public class OkHttpElasticClient implements ElasticClient {
    ...
    public SearchResponse search(final String name, final Query query) {
        final String dsl;
        try {
            dsl = QueryClause.toElasticQuery(query.getQuery(), mapper);
        } catch (final IOException e) {
            throw new ElasticException("Unable to generate Elastic query", e);
        }

        return execute(() -> {
            return new DelegatingCall<>(SearchResponse.class, api.search(name, dsl));
        });
    }
}
